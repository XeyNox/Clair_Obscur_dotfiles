...
- [ ] **Step 4: Vérifier qu'aucun anglais résiduel ne subsiste — dans le périmètre de CETTE tâche**

La section KEYBINDINGS appartient à la Tâche 5 et les window rules à la Tâche 6 : leurs commentaires anglais s'y traitent, pas ici. Le contrôle porte donc sur tout ce qui précède la section KEYBINDINGS :

```bash
awk '/---- KEYBINDINGS ----/{exit} {print NR": "$0}' hypr/.config/hypr/hyprland.lua \
  | grep -E '^\s*[0-9]+:\s*--.*\b(the|your|you|this|for more|Example|See)\b'
```

Expected: aucune sortie.

Le même contrôle **sur le fichier entier** est repoussé à la Tâche 7, quand les trois tâches de nettoyage auront toutes eu lieu. Le lancer ici retournerait des lignes qui ne sont pas de ton ressort.
...
- [ ] **Step 3 bis: Franciser les commentaires de la section KEYBINDINGS**

Cette section est la tienne : ses commentaires d'exemple anglais se traitent ici. La Tâche 4 les a délibérément laissés.

```bash
awk '/---- KEYBINDINGS ----/{f=1} /---- WINDOWS AND WORKSPACES ----/{f=0} f{print NR": "$0}' \
  hypr/.config/hypr/hyprland.lua \
  | grep -E '^\s*[0-9]+:\s*--.*\b(the|your|you|this|for more|Example|See|Requires)\b'
```

Traiter chaque ligne retournée : supprimer si le commentaire paraphrase le code, traduire en justifiant s'il apporte une information. Relancer jusqu'à sortie vide.
...
- [ ] **Step 3 bis: Franciser la section, et retirer une variable inutilisée**

Les commentaires anglais de la section WINDOWS AND WORKSPACES se traitent ici — la Tâche 4 les a laissés car ils n'étaient pas de son ressort. Notamment celui à l'intérieur de `suppress-maximize-events`.

`suppressMaximizeRule` est une variable locale sans usage depuis que son seul appel (`set_enabled(false)`, commenté) a été retiré en Tâche 4. La règle doit rester ; seule l'affectation inutile disparaît :
...
- [ ] **Step 4 bis: Aucun anglais résiduel dans TOUT le fichier**
